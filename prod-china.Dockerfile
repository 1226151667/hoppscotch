...
# 国内镜像：Go 二进制从 阿里云镜像 下载（原 go.dev）
RUN case "${TARGETARCH}" in amd64) GOARCH=amd64 ;; arm64) GOARCH=arm64 ;; *) echo "Unsupported arch: ${TARGETARCH}" && exit 1 ;; esac && \
  curl -fsSL "https://mirrors.aliyun.com/golang/go${GOLANG_VERSION}.linux-${GOARCH}.tar.gz" -o go.tar.gz
...
RUN mkdir -p /tmp/caddy-build && \
  # 国内镜像：GitHub 下载走 ghproxy.net 代理
  curl -fsSL -o /tmp/caddy-build/src.tar.gz https://ghproxy.net/https://github.com/caddyserver/caddy/releases/download/v2.11.4/caddy_2.11.4_src.tar.gz
...
# 国内镜像：从 npmmirror 下载 npm 包（原 registry.npmjs.org）
RUN curl -fsSL https://registry.npmmirror.com/npm/-/npm-11.17.0.tgz -o npm.tgz
# Verify checksum
...
RUN mkdir -p /tmp/pnpm-install && cd /tmp/pnpm-install && \
  # 国内镜像：从 npmmirror 下载 pnpm 和 import-meta-env（原 registry.npmjs.org）
  curl -fsSL https://registry.npmmirror.com/pnpm/-/pnpm-10.33.4.tgz -o pnpm.tgz && \
  curl -fsSL https://registry.npmmirror.com/@import-meta-env/cli/-/cli-0.7.4.tgz -o cli.tgz && \
